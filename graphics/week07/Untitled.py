...
def my_bilinear(img, x, y):
    ...
    interVal = img[floorY, floorX, :] * zz + img[floorY, floorX + 1, :] * zo +                img[floorY + 1, floorX, :] * oz + img[floorY + 1, floorX + 1, :] * oo
...
def feature_matching_RANSAC(img1, img2, keypoint_num=None, iter_num=500, threshold_distance=10):
    ...
    for i in range(iter_num):
        ...
        affineMatrix = np.array([[X[0][0], X[1][0], X[2][0]],
                  [X[3][0], X[4][0], X[5][0]],
                  [0, 0, 1]])
        
        
        # 3. check inlier for every matches
        count = 0
        for idx, match in enumerate(matches):
            trainInd = match.trainIdx
            queryInd = match.queryIdx
            ...
            xy_prime = np.array([[x_prime,y_prime,1]]).T
            xy_ = (np.linalg.inv(affineMatrix)).dot(xy_prime)
            x_ = xy_[0, 0]
            y_ = xy_[1, 0]
            
            distance = L2_distance(np.array([x_,y_]),np.array([x_,y_])) 
            if distance < threshold_distance:
                count+=1
        inliers.append(count)
        M_list.append(affineMatrix)

    maxIndex = np.argmax(inliers)
    best_M = M_list[maxIndex]
...
def main():
    ...
    cv2.imshow('result RANSAC 201902765', result_RANSAC)
