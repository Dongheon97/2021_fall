...
public class MiniCPrintListener extends	MiniCBaseListener {
    ...
    private boolean isBinaryOperation(MiniCParser.ExprContext ctx) { // 이진연산인지 확인
        return ctx.getChildCount() == 3 && ctx.getChild(1) != ctx.expr();
    }

    private boolean isPreOperation(MiniCParser.ExprContext ctx) { // 전위연산인지 확인
        return (ctx.getChildCount() == 2 && ctx.getChild(0) != ctx.expr());
    }

    @Override
    public void enterParam(MiniCParser.ParamContext ctx) { //파라미터 가져오기
    ...
    @Override
    public void enterExpr(MiniCParser.ExprContext ctx) { //Expr일 경우 수행
    ...
    @Override
    public void exitExpr(MiniCParser.ExprContext ctx) { //Expr 연산 수행
    ...
    @Override
    public void enterFun_decl(MiniCParser.Fun_declContext ctx) { //함수로 들어갈 때
        printDepth();
        depth ++;
    ...
    @Override
    public void exitFun_decl(MiniCParser.Fun_declContext ctx) { // Func 종료시 수행
        depth --;
        printDepth();
    ...
    @Override
    public void enterIf_stmt(MiniCParser.If_stmtContext ctx) { // if문 처리
        printDepth();
        depth ++;
        statement = true;
    ...
    @Override
    public void exitIf_stmt(MiniCParser.If_stmtContext ctx) { // if문 종료
        depth --;
        printDepth();
